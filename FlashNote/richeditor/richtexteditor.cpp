...
void RichTextEditor::dropImage(const QImage& image, const QString& format)
{
	QByteArray bytes;
...
void RichTextEditor::insertFromMimeData(const QMimeData* source)
{
	if (source->hasImage())
	{
		...
		if (!format.isEmpty()) {
			...
			dropImage(qvariant_cast<QImage>(source->imageData()), "JPG"); // Sorry, ale cokoli jin¨¦ho dlouho trv¨¢
			return;
		}
	}
